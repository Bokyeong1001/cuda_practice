...
import sys
sys.path.append("/home/moon/bokyeong/cuda-python/")
import ctypes
import math
import numpy as np
...
def main():
    N = 4
    size = N * N * np.dtype(np.int32).itemsize
    ...
    kernelHelper = common.KernelHelper(matmul_kernel, int(cuDevice))
    _gpu_matrix_mult = kernelHelper.getFunction(b'gpu_matrix_mult')

    # Allocate input vectors h_A and h_B in host memory
    h_A = np.zeros(N*N).astype(dtype=np.int32)
    h_B = np.zeros(N*N).astype(dtype=np.int32)
    h_C = np.random.rand(N*N).astype(dtype=np.int32)
    h_Z = np.random.rand(N*N).astype(dtype=np.int32)
    ...
    BLOCK_SIZE = N
    grid_rows = (N + BLOCK_SIZE - 1) / BLOCK_SIZE
    ...
    checkCudaErrors(cuda.cuLaunchKernel(_gpu_matrix_mult,
