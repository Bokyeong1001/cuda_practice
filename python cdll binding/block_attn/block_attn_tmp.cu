...
//nvcc -Xcompiler -fPIC -shared -lcusparse -o attn.so attn.cu
...
extern "C" {
void attn(float *hQuery, float *hKey, float *hValue, float *hAttn, float *hOut, int *hOffsets, int *hColumns, int seq_len, int emb_dim, int nnz, int num_batches)
{
    // Host problem definition
    int   input_size       = seq_len * emb_dim;

    int   *d_offsets, *d_columns;
    float *dQuery, *dKey, *dValue, *dAttn, *dOut;
    ...
    spmm(handle, dBuffer, dAttn, dValue, dOut, d_offsets, d_columns, seq_len, emb_dim, nnz, num_batches);
